...
def calculate_bars_since_crossover(df, last_crossover_index):
    """
    Calculate the number of bars since the last crossover.
    Uses the previous closed bar (len(df) - 2) as the current bar index.

    Args:
        df (DataFrame): DataFrame with candle data
        last_crossover_index (int or None): Index of last crossover

    Returns:
        int or None: Bars elapsed since last crossover or None if unknown
    """
    current_index = len(df) - 2  # use previous closed bar
    if last_crossover_index is None:
        return None
    return current_index - last_crossover_index
    
def check_ema_crossover_signal(df, short_period=9, long_period=20):
    global last_processed_index, last_crossover, last_crossover_index
    ...
    if current_index == last_processed_index:
        return None, None, None  # Skip processing and printing for repeated calls
    ...
    readable_time = pd.to_datetime(candle_time, unit='s', utc=True)
    readable_time_ist = readable_time.tz_convert('Asia/Kolkata')

    print(readable_time_ist)
    ...
    if prev_short <= prev_long and curr_short > curr_long:
        ...
        last_crossover_index = current_index             #here changed from len(df) - 1 to current index changesd this at two places
        new_crossover = True
    elif prev_short >= prev_long and curr_short < curr_long:
        signal = "📉 SELL"
        last_crossover = 'bearish'
        last_crossover_index = current_index
        new_crossover = True

    # Calculate bars since last crossover through new function
    bars_since_crossover = calculate_bars_since_crossover(df, last_crossover_index)
    ...
    print(f"Time: {readable_time_ist}")
    print(f"EMA {short_period}: {prev_short:.4f}, EMA {long_period}: {prev_long:.4f}")
